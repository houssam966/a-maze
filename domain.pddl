(define (domain maze)
     ...
     (:action goTo
      ...
      :precondition (and (> (playerHealth) 0) (on ?p ?f) (atLocation ?p ?from) (isConnected ?from ?to) (not (isLocked ?from ?to)))
     ...
     (:action pickUp
      ...
      :precondition (and (> (playerHealth) 0) (atLocation ?p ?j) (atLocation ?i ?j) (on ?p ?platform) (on ?i ?platform)(<(inventoryCount) 
                    (maxInventorySize))(not (carryItem ?p ?i)))
     ...
     (:action push
      ...
      :precondition (and (> (playerHealth) 0) (on ?p ?f) (on ?b ?f) (atLocation ?p ?from) (atLocation ?b ?from) 
     ...
     (:action climb
      ...
      :precondition (and (atLocation ?p ?j) (atLocation ?to ?j) (on ?p ?from ) (= (-(platformLevel ?to) 
                    (platformLevel ?from)) 1) (> (playerHealth) 0))
     ...
     (:action grab
      ...
      :precondition (and (atLocation ?p ?j) (on ?p ?current) (on ?i ?from) (atLocation ?from ?j) (= (-(platformLevel ?from) 
                    (platformLevel ?current)) 1) (> (playerHealth) 0))
     ...
     ; Arguments:
     ; ?player {Living}: the player of the game
     ; ?monster {Living}: the monster of the game
     ; ?platform {Platform}: current platform of player and monster
     ; ?j {Junction}: current location of player and monster
     ; ?w {Item}: weapon to kill monster
     ; ?k {Item}: the key that can be used to unlock a box
     (:action attack
      :parameters (?p - Player ?m - Monster ?w - Weapon ?k - Key ?j - Junction ?platform - Platform)
      :precondition (and (atLocation ?p ?j) (atLocation ?m ?j) (carryItem ?p ?w) (on ?p ?platform) (on ?m ?platform)
     ...
     ; j{Junction}: the current location of the player, box and item
     ; platform {Platform}: used place any locatable object on top of a platform
     (:action unlockBox
      :parameters (?p - Player ?k - Key ?b - Box ?j - Junction ?platform - Platform)
      :precondition (and (atLocation ?p ?j) (atLocation ?b ?j)  (on ?p ?platform) (on ?b ?platform) (carryItem ?p ?k) 
                    (not(isUnlocked ?b)) (> (playerHealth) 0))
